...
int main(int argc, char **argv)
{
    ...
    s1 = argv[2];
    s2 = argv[3];
    if (s1.empty()) // checkit dk li an3ado bih
    {
        std::cout << "Error: string to replace (in the file given) cannot be empty" << std::endl;
        return 1;
    }
    std::ifstream file(argv[1]);    // i checked wach file kay exister
    if (!file)
    {
        std::cout << "Couldn't open input file." << std::endl;
		return (1);
    }
    std::string content = std::string(        //copit lfile l string content
        std::istreambuf_iterator<char>(file),
        std::istreambuf_iterator<char>()
    );

    std::string result;  // will hold the final content
    size_t pos = 0;
    size_t found;
